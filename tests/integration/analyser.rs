...
fn infer_example3() {
    ...
    _results
"#;
    //let case = r#"private _dict2 = [_dict, "a"] call EFUNC(get);"#;

    let path = "tests/integration/dictionary/addons/dictionary/fnc_serialize.sqf".into();
    analyze(&parse(tokens(case).unwrap()), path).unwrap();
}
